  ...
  return (
    <StyledServersContainer>
        <div className="server_buttons_container">
          <button className={server === "vidsrc.to" ? "active" : ""} onClick={() => setSource(showId ? `https://vidsrc.to/embed/tv/${showId}/${selectedSeason}/${selectedEpisode}` : `https://vidsrc.xyz/embed/movie?tmdb=${movieId}`)}>Server 1</button>
          <button className={server === "vidsrc.xyz" ? "active" : ""} onClick={() => setSource(showId ? `https://vidsrc.xyz/embed/tv/${showId}/${selectedSeason}/${selectedEpisode}` : `https://vidsrc.to/embed/movie/${movieId}`)}>Server 2</button>
